...
import org.bohdanrakov.jackcompiler.compilationengine.symboltable.SymbolTable;
...
public class JackToVMCompilationEngine implements CompilationEngine {
    ...
    public List<String> compileClass(List<Token> tokens) {
        ...
        result.add(classIdentifierToXML(currentToken));
    ...
    private void compileClassVariableDeclaration(List<String> result) {
        ...
        if (currentToken.getStringValue().equals("static")) {
            compileStaticVariableDeclaration();
        } else {
            compileFieldDeclaration();
        }

        proceedNextToken();
        validateTokenType(KEYWORD, IDENTIFIER);

        if (currentToken.getTokenType() == IDENTIFIER) {
            result.add(classIdentifierUsedToXML(currentToken));
        } else {
            result.add(tokenToXML(currentToken));
        }
        ...
        proceedNextToken();
        validateTokenType(IDENTIFIER);
        //symbolTable.define(currentToken.getStringValue(), currentToken.getTokenType());
        result.add(identifierDefinedToXML(currentToken));
    ...
    private void compileFieldDeclaration() {
        result.add(tokenToXML(currentToken));
    }

    private void compileStaticVariableDeclaration() {
        result.add(tokenToXML(currentToken));
    }
    ...
    private String identifierDefinedToXML(Token token) {
        String stringValue = token.getStringValue();
        symbolTable.kindOf(stringValue);
        symbolTable.typeOf(stringValue);
        return "<identifier> " + stringValue + " </identifier>";
    }


    private String identifierUsedToXML(Token token) {
        String stringValue = token.getStringValue();
        symbolTable.kindOf(stringValue);
        symbolTable.typeOf(stringValue);
        return "<identifier> " + stringValue + " </identifier>";
    ...
    private String classIdentifierToXML(Token token) {
        return "<identifier> " + token.getStringValue() + ", category:class, defined</identifier>";
    }

    private String subroutineIdentifierToXML(Token token) {
        return "<identifier> " + token.getStringValue() + ", category:subroutine, defined</identifier>";
    }

    private String classIdentifierUsedToXML(Token token) {
        return "<identifier> " + token.getStringValue() + ", category:class, used</identifier>";
    }

    private String subroutineIdentifierUsedToXML(Token token) {
        return "<identifier> " + token.getStringValue() + ", category:subroutine, used</identifier>";
    }
}
